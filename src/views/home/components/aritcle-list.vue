...
export default {
  ...
  data () {
    return {
      ...
      datatimestamp: null, // 请求下一页数据的时间戳
      ...
      refreshSuccessText: '刷新成功' // 刷新失败的提示
    }
  },
  methods: { // 初始化或滚动到底部的时候会触发调用 onLoad 事件
    async onLoad () {
      try {
        ...
        const { data } = await getArticles({
          ...
          timestamp: this.datatimestamp || Date.now() // 如果本地有时间戳则先用本地的，没有就取 Date.now() （针对一开始进入页面的情况）
        ...
        // 4、判断数据是否全部加载完成
        if (results.length) {
          // 更新获取下一页数据的时间戳  pre_timestamp：请求前一页历史数据的时间戳
          this.datatimestamp = data.data.pre_timestamp
        } else {
          this.finished = true
        }
      } catch (err) {
    ...
    async onRefresh () {
      try {
        ...
        // 模拟随机加载失败的情况
        if (Math.random() > 0.5) {
          JSON.parse('jdlkajdkdlk')
        }

        const { results } = data.data
        // 2、把请求结果数据放到 list 数组中最前面
