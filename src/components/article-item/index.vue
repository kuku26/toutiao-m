<template>
<!-- Cell 单元格的 to 属性和 VueRooter 中的 RouterLink 导航组件的 to 属性用法是一样的 -->
<!-- :to="/article/ + 'article.art_id' -->
<!-- :to="/article/${article.art_id} -->
<!-- :to="{name: 'article', params: {articleId: article.art_id}}" -->
<!-- name: 根据路由名称进行跳转 -->
<!-- params: 传递路由动态参数 -->
<!-- articleId：属性名：路由路径中设计的动态参数名称 -->
  <van-cell class="article-item" :to="{name: 'article', params: {articleId: article.art_id}}">
    ...
    <div slot="label">
      ...
      <van-image  slot="default" v-if="article.cover.type === 1" class="rigth-cover" fit="cover"
